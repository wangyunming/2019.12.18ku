 ...
 * ---------------------------------------------------------------------------------
 * mac 1    48    6      281,474,976,710,655
 * mac 2    49           562,949,953,421,311    9      11,694,146,092,834,140
 * ------
 * MAC      RAN(12) * E15 + mac
 * ---------------------------------------------------------------------------------
 * time 0                13,054,752,000   (from 2011.05.16 100ms)
 * time 1                28,833,984,000   (from 2061.05.16 100ms)
 *                                              7           3,142,742,836,021
 * ------
 * TIME     RAN(32) * E11 + time
 * ---------------------------------------------------------------------------------
 * pid      16    2      65535                  3                     226,980
 * ------
 * PID      pid
 * ---------------------------------------------------------------------------------
 * seq      26           67,108,864             5                 844,596,300
 * ------
 * SEQ      RAN(8) * E8 + seq
 * ---------------------------------------------------------------------------------
 * ip       32    4      4,294,967,296          6              51,520,374,361
 * ------
 * IP       RAN(5) * E10 + ip
 * ================================================================================= *
 *
...
public class Token {
	...
	private final static Log log = LogFactory.getLog(Token.class);

    final static char[] BASE61 = "3ibqZmxNpXOjlHIPQVSTUhWCBgFDctuYAzKd1fenyE6957rwMaLJ2kR4vso8G".toCharArray();
    private final static int[] BASE61_MAPPING = { 32,24,23,27,41,26,60,13,14,51,34,50,48,7,10,15,16,54,18,19,20,17,22,9,31,4,49,2,28,35,38,37,25,21,1,11,53,12,5,39,58,8,3,46,57,29,30,56,47,6,40,33,36,52,0,55,44,42,45,59,43 };
    ...
    private final static AtomicInteger[] SEN = {
    ...
    public static Token getToken() {
        return new Token(ServletUtil.getRemoteIp());
    }
	...
	public static boolean checkToken(String value) {
        ...
        for (int i = DATA_LENGTH, j = 0; i < chs.length; i++, j++) {
            if (chs[i] != hash[j]) {
                log.warn("value hash is invalid, value hash: [" + new String(chs, DATA_LENGTH, TOKEN_LENGTH - DATA_LENGTH) +
                		"], calculate hash: [" + new String(hash) + "], token: [" + value + "]");
                return false;
            }
        }
        return true;
    ...
    public String toString() {
        return "Token [mac=" + mac + ", tokenTime=" + String.format("%tF %<tT", getTokenTime()) + ", pid="
                + pid + ", sequence=" + sequence + ", clientIp=" + clientIp
                + "]";
    }
