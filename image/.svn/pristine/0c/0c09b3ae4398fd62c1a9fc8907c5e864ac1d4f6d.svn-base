...
/**
 * Assertion utility class that assists in validating arguments. Useful for identifying programmer errors
 * early and clearly at runtime.
 * 
 * <p>
 * For example, if the contract of a public method states it does not allow <code>null</code> arguments,
 * Assert can be used to validate that contract. Doing this clearly indicates a contract violation when it
 * occurs and protects the class's invariants.
 * 
 * <p>
 * Typically used to validate method arguments rather than configuration properties, to check for cases that
 * are usually programmer errors rather than configuration errors. In contrast to config initialization code,
 ...
 * </pre>
 * 
 * Mainly for internal use within the framework; consider Jakarta's Commons Lang >= 2.0 for a more
...
public class Assert {
	...
	/**
	 * Assert that the given String is not empty; that is, it must not be <code>null</code> and not the empty
	 * String.
	 * 
	 * <pre class="code">
	...
	/**
	 * Assert that the given String has valid text content; that is, it must not be <code>null</code> and must
	...
	/**
	 * Assert that a collection has elements; that is, it must not be <code>null</code> and must have at least
	 * one element.
	 * 
	 * <pre class="code">
	...
	/**
	 * Assert that a Map has entries; that is, it must not be <code>null</code> and must have at least one
	 * entry.
	 * 
	 * <pre class="code">
	 ...
	 *            the object to check
	 * @param message
	 *            a message which will be prepended to the message produced by the function itself, and which
	 *            may be used to provide context. It should normally end in a ": " or ". " so that the
	...
	public static void isInstanceOf(Class<?> type, Object obj, String message) {
		...
		if (!type.isInstance(obj)) {
			log.error(message + "Object of class [" + (obj != null ? obj.getClass().getName() : "null")
	...
	public static void isAssignable(Class<?> superType, Class<?> subType, String message) {
		...
		if (subType == null || !superType.isAssignableFrom(subType)) {
			log.error(message + subType + " is not assignable to " + superType);
	 ...
	 * <code>false</code>.
	 * <p>
	 * Call {@link #isTrue(boolean)} if you wish to throw {@link IllegalArgumentException} on an assertion
	 * failure.
	 * 
	 * <pre class="code">
	...
	public static void hasLength(Collection<?> collection, String message) {
		if (collection.size())
		if (!StringUtils.hasLength(text)) {
			log.error(message);
			throw new IllegalArgumentException(message);
